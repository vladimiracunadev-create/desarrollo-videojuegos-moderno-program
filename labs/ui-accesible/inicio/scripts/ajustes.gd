extends Node
## EJERCICIO — Los ajustes del jugador (Autoload).
##
## Arranca ya: verás el menú… en clave. Botones que ponen "MENU_JUGAR" en vez de
## "Jugar". Ese es el síntoma clásico de la localización sin conectar, y tu
## punto de partida. Los TODO van en este orden:
##
##   TODO 1 · aplicar idioma y escala de texto   (clases 190, 196, 197)
##   TODO 2 · guardar y cargar los ajustes       (clase 199)
##   TODO 3 · remapear una tecla                 (clase 195)
##   TODO 4 · los textos hechos por código       (clase 197) → scripts/hud.gd
##   TODO 5 · foco y navegación                  (clases 192, 195) → scripts/menu.gd
##
...
func aplicar() -> void:
	# TODO 1: aplica los dos ajustes a TODA la UI de una vez:
	#   TranslationServer.set_locale(idioma)
	#   if _tema != null:
	#       _tema.default_font_size = int(round(FUENTE_BASE * escala_texto))
	#
	# Fíjate en lo que NO hay que hacer: recorrer los Label uno a uno con
	# add_theme_font_size_override(). Tocar el tema del proyecto cambia la UI
	# entera; ir nodo por nodo es la forma segura de olvidarte de uno (clase 190).
	cambiaron.emit()
...
func remapear(accion: String, evento: InputEventKey) -> void:
	# TODO 3: cambia la tecla de una acción:
	#   if not InputMap.has_action(accion): return
	#   InputMap.action_erase_events(accion)   # fuera la anterior
	#   InputMap.action_add_event(accion, evento)
	#   guardar()
	#
	# Sin el erase, la acción acaba con DOS teclas y la vieja sigue funcionando.
	pass
...
func guardar() -> void:
	# TODO 2a: guarda idioma, escala_texto y subtitulos en RUTA con ConfigFile:
	#   var cfg := ConfigFile.new()
	#   cfg.set_value("ui", "idioma", idioma)
	#   ...
	#   cfg.save(RUTA)
	#
	# Un ajuste de accesibilidad que no persiste obliga a reconfigurarlo en cada
	# arranque. Quien lo necesita, lo necesita SIEMPRE.
	pass


func cargar() -> void:
	# TODO 2b: carga lo guardado; si no hay archivo, no toques nada (primera vez):
	#   var cfg := ConfigFile.new()
	#   if cfg.load(RUTA) != OK: return
	#   idioma = str(cfg.get_value("ui", "idioma", idioma))
	#   ...
	#
	# Pon siempre un valor por defecto en get_value(): un .cfg de una versión
	# vieja puede no tener la clave que acabas de añadir.
	pass
